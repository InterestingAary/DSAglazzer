...
export const ProgressAnalytics: React.FC<ProgressAnalyticsProps> = ({ userStats, nodes }) => {
  ...
  return (
    <motion.section className="space-y-6" variants={containerVariants} initial="hidden" animate="visible">
      <motion.h2 className="font-heading text-xl font-bold text-text-primary gradient-text" variants={itemVariants}>
        PROGRESS ANALYTICS
      </motion.h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <motion.div className="spatial-card p-6" variants={itemVariants}>
          ...
          <div className="relative h-52 flex items-center justify-center">
            <svg className="w-40 h-40 -rotate-90" viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="40" fill="none" stroke="url(#gaugeGradient)" strokeWidth="8" strokeLinecap="round" strokeDasharray={251.2} strokeDashoffset={251.2 - (251.2 * accuracy) / 100} className="transition-all duration-1000 ease-out" />
              <defs>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="text-4xl font-black text-text-primary glow-text-accent">{accuracy}%</span>
        ...
        <motion.div className="spatial-card p-6" variants={itemVariants}>
          ...
          <div className="space-y-5">
            {[
              { label: 'Easy', solved: userStats.easySolved, total: userStats.easyTotal, color: '#6366f1', glow: 'rgba(99,102,241,0.4)' },
              { label: 'Medium', solved: userStats.mediumSolved, total: userStats.mediumTotal, color: '#f59e0b', glow: 'rgba(245,158,11,0.4)' },
              { label: 'Hard', solved: userStats.hardSolved, total: userStats.hardTotal, color: '#ef4444', glow: 'rgba(239,68,68,0.4)' },
            ].map((d) => (
              <div key={d.label}>
            ...
            {nodes.slice(0, 8).map((node) => (
              <div key={node.id}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[10px] font-mono text-text-muted">{Math.round(node.progress)}%</span>
                </div>
