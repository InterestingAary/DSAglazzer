// CodeChef content script - Detects solved problems
...
function isProblemPage() {
  return location.pathname.includes('/problems/') && !location.pathname.includes('/status');
}
...
function checkSubmissionStatus() {
  ...
  // Check for "Accepted" or "AC" in page
  const pageText = document.body.innerText;
  if (pageText.includes('Accepted') || pageText.includes('AC ') || pageText.includes('Successful')) {
    extractAndSendProblemData();
  }
}
      ...
      if (mutation.type === 'childList') {
        for (const node of mutation.addedNodes) {
          if (node.nodeType === Node.ELEMENT_NODE) {
            if (isSuccessElement(node) || node.querySelector?.('[class*="accepted"], [class*="ac"]')) {
...
function extractTitle() {
  ...
  for (const selector of selectors) {
    ...
    if (el && el.innerText.trim()) {
      return el.innerText.trim().replace(/^[A-Z]\d+\s*/, '');
    }
  }
